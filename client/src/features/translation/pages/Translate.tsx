...
export default function TranslatePage() {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#06121F]">
      {/* Background decorations */}
      <div className="pointer-events-none absolute inset-x-0 top-0 h-96 bg-[radial-gradient(ellipse_at_top,_rgba(139,92,246,0.08),_transparent_60%)]" />
      <div className="pointer-events-none absolute -top-24 right-0 h-72 w-72 rounded-full bg-violet-500/10 blur-3xl" />
      <div className="pointer-events-none absolute left-0 top-32 h-64 w-64 rounded-full bg-sky-500/10 blur-3xl" />
      <div className="pointer-events-none absolute bottom-0 right-0 h-80 w-80 rounded-full bg-emerald-500/8 blur-3xl" />

      <div className="relative mx-auto max-w-4xl px-4 py-6 sm:px-6 lg:px-8 lg:py-10">
        {/* ── Page header ───────────────────────────────────────────────── */}
        <header className="mb-8">
          <div className="inline-flex items-center gap-2 rounded-full border border-violet-400/30 bg-violet-500/15 px-4 py-2 text-xs font-medium uppercase tracking-[0.24em] text-violet-300 shadow-sm backdrop-blur">
          ...
          <div className="mt-4 flex items-end justify-between gap-4">
            <div>
              <h1 className="text-3xl font-semibold tracking-tight text-[#F8FAFC] sm:text-4xl lg:text-5xl">
                Real-time voice
              ...
              <p className="mt-3 max-w-2xl text-sm leading-6 text-[#94A3B8] sm:text-base">
                ...
                <span className="font-medium text-[#F8FAFC]">Gemini AI</span>.
              </p>
            </div>
            ...
            {isActive && (
              <button
                ...
                className="flex-shrink-0 rounded-2xl border border-white/15 bg-white/10 px-4 py-2.5 text-sm font-medium text-[#F8FAFC] shadow-sm transition hover:border-red-400/40 hover:bg-red-500/20 hover:text-red-300 focus:outline-none focus:ring-2 focus:ring-red-400 focus:ring-offset-0"
              >
                ↩ New Session
              </button>
            )}
        ...
        {!isActive && (
          <div className="mb-6 grid grid-cols-1 gap-3 sm:grid-cols-3">
            {[
              ...
              <div
                key={i}
                className="flex items-start gap-3 rounded-2xl bg-[#0E1B2D] p-4 ring-1 ring-white/8 shadow-sm"
              >
                <div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-xl bg-white/5">
                  {step.icon}
                </div>
                <div>
                  <p className="text-xs font-semibold text-[#F8FAFC]">{step.title}</p>
        ...
        {/* ── Footer note ────────────────────────────────────────────────── */}
        <footer className="mt-8 text-center text-xs text-[#64748B]">
